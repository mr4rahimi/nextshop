...
> ⚠️ **تخفیف باسلام مدل «کمپین» دارد، نه فیلد روی محصول.** درصد و «چند روز فعال
> بماند» می‌گیرد؛ تاریخ شروع و پایان ندارد و حذف تخفیف فراخوانی جداگانه می‌خواهد.
> سازوکار کامل و تله‌هایش در [discounts.md](../discounts.md) بخش «نگاشت فیلدها
> در باسلام» آمده — پیش از دست زدن به قیمت حتماً بخوانیدش.

## میزبان‌ها

| میزبان | کاربرد |
|---|---|
| `https://auth.basalam.com` | دریافت و تمدید توکن |
| `https://openapi.basalam.com` | فهرست محصولات، سفارش‌ها، **کمپین تخفیف** |
| `https://core.basalam.com` | بروزرسانی گروهی قیمت و موجودی (`PATCH /v3/vendors/{id}/products`) |

هر دو میزبان زنده‌اند و هر کدام کار خودشان را می‌کنند. اشتباه گرفتنشان همان
خطایی بود که سرویس تخفیف را از کار انداخته بود.

## کمپین تخفیف

```
POST   https://openapi.basalam.com/v1/vendors/{vendorId}/discounts
       { "product_filter": { "product_ids": [123] },
         "discount_percent": 20, "active_days": 5 }

DELETE https://openapi.basalam.com/v1/vendors/{vendorId}/discounts
       { "product_filter": { "product_ids": [123] } }
```
...
استفاده می‌کنیم تا کمپین دقیقاً روی همان محصول نگاشت‌شده بنشیند.

## قیمت

`primary_price` قیمت خط‌خورده است و فقط وقتی پر می‌شود که کمپین فعال باشد.
`price` چیزی است که خریدار می‌پردازد. وقتی کمپین می‌سازیم باید **قیمت اصلی** را
بفرستیم، چون باسلام خودش تخفیف را رویش حساب می‌کند.

قیمت‌ها در API به **ریال** است و داخلی همه‌جا تومان.
