...
> ⚠️ **هدر احراز هویت عوض شده است.** از ۲۱ شهریور ۱۴۰۵ تپسی فقط
> `Tapsi-Shop-Hub-Authorization` را می‌پذیرد؛ املای قدیمی
> `TapsiShop.Hub.Authorization` و `TapsiShop.Hub.Webhook-Authorization` حذف می‌شوند.
> کد ما در دوره‌ی گذار هر دو را می‌فرستد و هر دو را می‌پذیرد.
>
> منبع حقیقت: `webhook.v.0.3.pdf` در همین پوشه. متن خام آن در انتهای این فایل
> ضمیمه است و به‌خاطر استخراج از PDF درهم‌ریخته می‌خوانَد — خلاصه‌ی زیر را ملاک بگیرید.
...
| فرم ادمین | `/admin/integration/connections/tapsi` |
| آداپتور | `lib/integration/adapters/marketplace/tapsi.adapter.ts` |
| وب‌هوک ورودی | `app/api/integration/webhooks/tapsi/route.ts` |
...
همین نام هدر در **هر دو جهت** استفاده می‌شود: درخواست‌های خروجی ما به تپسی، و
درخواست‌های ورودی تپسی به وب‌هوک ما. ولی **توکن‌شان یکی نیست** — توکن وب‌هوک در
پنل وندور جداگانه انتخاب می‌شود و در فرم اتصال هم فیلد جداگانه دارد.

## بروزرسانی قیمت و موجودی

`PUT /products` — حداکثر ۵۰ قلم در هر درخواست.

```json
{ "products": [
  { "id": "<sku فروشنده>", "stock": 10, "price": 200000,
    "specialPrice": 100000, "referenceCode": "<کد ما>" }
] }
```

- `id` همان **SKU سمت فروشنده** است، نه شناسه‌ی محصول در تپسی
- قیمت‌ها به **ریال** و باید مضرب ۱۰ باشند (ما تومان را در ۱۰ ضرب می‌کنیم)
- `price` قیمت اصلی، `specialPrice` قیمت نهایی بعد از تخفیف
- برابر گذاشتن این دو یعنی «تخفیف نداشته باشد»
- `referenceCode` عیناً در پاسخ برمی‌گردد و کلید تطبیق نتیجه‌ی هر قلم است

پاسخ برای هر قلم `status`، `messages`، `currentOriginalPrice`،
`currentFinalPrice` و `currentOnHandQuantity` می‌دهد.

## تخفیف

تپسی **بازه‌ی زمانی و موجودی تخفیف‌دار ندارد** — فقط یک قیمت نهایی می‌گیرد.
پس تاریخ شروع و پایانی که در پنل ما وارد می‌شود را خود پنل اجرا می‌کند: با باز
شدن بازه قیمت تخفیف‌دار و با بسته شدنش قیمت اصلی ارسال می‌شود. سازوکارش در
[discounts.md](../discounts.md) بخش «اجرای بازه» توضیح داده شده.

## وب‌هوک سفارش

تپسی با `POST` به آدرس ثبت‌شده در پنل وندور خبر می‌دهد.

| فیلد | معنی |
|---|---|
| `orderDetail.changeType` | ۱ = خرید، ۲ = لغو |
| `items[].productId` | SKU سمت فروشنده |
| `items[].orderItemId` | شناسه‌ی قلم سفارش — کلید یکتای ما برای جلوگیری از تکرار |
| `items[].quantity` | ‎−۱ در خرید، ‎+۱ در لغو |
| `items[].changeType` | ۱ = کسر بابت خرید، ۲ = افزایش بابت لغو |
| `items[].requestId` | شناسه‌ی درخواست، برای تشخیص ارسال تکراری |
| `items[].finalPrice` | قیمت نهایی به ریال |

پاسخ ما باید `200` با بدنه‌ی `{"succeed": true, "message": "..."}` باشد. تا وقتی
`succeed: true` نگیرد همان سفارش دوباره فرستاده می‌شود، و اگر خطا از حد مجاز
بگذرد تپسی ارسال به سرویس ما را **خودکار غیرفعال می‌کند**.

## سرویس‌های دیگر

| مسیر | کاربرد |
|---|---|
| `POST /refresh-token` | توکن جدید بر پایه‌ی توکن فعلی — پیش‌فرض انقضا ۶ ماه |
| `GET /courier/{pickupCode}` | مشخصات سفیر برای احراز هویت هنگام تحویل |
| `PUT /review-courier` | تأیید یا رد سفیر |

این سه هنوز در آداپتور پیاده نشده‌اند.
