...
### ✅ فاز ۵ — بازطراحی معماری نگاشت + Wizard چندمرحله‌ای

**تغییر معماری بنیادی:**
- `IntegProductMapping` حذف شد
- `IntegMapping` (گروه یکپارچه) + `IntegMappingLink` (یک لینک به هر پلتفرم) جایگزین شد
- فروشگاه یکی از پلتفرم‌هاست (platformCode="shop") — نه entity اصلی
- هر نگاشت می‌تواند 0 یا N لینک به هر پلتفرم داشته باشد

**تغییرات فنی:**
- Migration: `IntegMapping` + `IntegMappingLink` (upsert @@unique بر `[mappingId, platformCode]` و `[platformCode, externalId]`)
- `mapping.ts`: `runAutoMatch` و `approveSuggestion` به مدل جدید آپدیت شد
- `worker.ts`: `syncAllStock` و `syncAllPrice` از `IntegMappingLink` استفاده می‌کند
- API: `GET/POST/DELETE /api/integration/mapping` + `POST/DELETE /api/integration/mapping/link`
- `IntegPlatformProduct`: ذخیره خام محصولات هر پلتفرم برای نمایش در UI

**صفحات جدید:**
- `Wizard نگاشت`: چندمرحله‌ای — هر مرحله یک پلتفرم (فروشگاه، حسابان، باسلام، ...) با گزینه «ندارم»
- `تب نگاشت‌ها`: نمایش گروه‌های نگاشت با همه پلتفرم‌هایشان
- `/admin/integration/product-suggestions`: پیشنهادات اضافه کردن محصول — نگاشت‌های ناقص با دکمه «ایجاد در فروشگاه» یا «لینک کردن»

### 🔜 مرحله بعدی
→ **فاز ۶**: Adapterهای بعدی (دیجیکالا، دیوار، اسنپ‌شاپ، تپسی‌شاپ)
...
### فاز ۱ — اتصال وب‌حسابان ✅ تکمیل شد (۱۴۰۵/۰۴/۱۰) + bugfix (۱۴۰۵/۰۴/۱۱)
- [x] `HesabanAdapter` (testConnection + fetchProducts + updateStock + updatePrice)
- [x] صفحه تنظیمات `/admin/integration/connections/hesaban`
- [x] job: `TEST_CONNECTION`
- [x] job: `SYNC_ALL_STOCK` از حسابداری به فروشگاه
...
> مستندات: `https://app.hesabanweb.com/swagger/index.html`  
> OpenAPI JSON: `hesabanweb.json` (نسخه v3 — OpenAPI 3.0.4)

> **⚠️ مهم — BASE URL**: آدرس پایه API برابر است با `https://app.hesabanweb.com` (نه `hesabanweb.com`)

### احراز هویت
- **نوع**: JWT Bearer Token
- **هدر**: `Authorization: Bearer {token}`
- **دریافت توکن**: کاربر از پنل وب‌حسابان → تنظیمات → API توکن می‌گیرد
- فرمت credentials در DB: `{ "token": "...", "storageId": "..." }` (storageId اختیاری)
- توکن حاوی اطلاعات DB است (DBID، DBC) — نیازی به هدر جداگانه نیست
...
# نسخه ۲.۲۵.۱ — سه خرابی خاموش در یکپارچه‌سازی (۱۴۰۵/۰۶/۰۳)

سه ایراد که هیچ‌کدام خطا نمی‌دادند و فقط از روی لاگ‌های `IntegLog` و صف `IntegJob`
قابل تشخیص بودند. هر سه روی مای‌مونتا فعال بودند.

## ۱. job راکد، کل سینک را می‌کشد
اگر پروسه وسط اجرای یک job ری‌استارت شود، آن job برای همیشه `PROCESSING` می‌ماند.
`ensureScheduledJobs` وجود یک job در `PENDING/PROCESSING` را «در حال اجرا» تفسیر
می‌کند و دیگر هرگز job جدیدی از آن نوع صف نمی‌کند.

روی مای‌مونتا یک `SYNC_ALL_STOCK` از ۱۴۰۵/۰۵/۰۶ در `PROCESSING` گیر کرده بود و
**۲۸ روز موجودی از حسابداری همگام نشد** — نه در نگاشت، نه در هیچ بازارگاهی.

**رفع:** `reclaimStaleJobs()` در `worker.ts` — هر job بیش از ۳۰ دقیقه در
`PROCESSING` به صف برمی‌گردد (یا اگر attempts تمام شده `FAILED` می‌شود).
**قبل از** `ensureScheduledJobs` صدا زده می‌شود، وگرنه همان چرخه دوباره آن را
«در حال اجرا» می‌بیند.

> ⚠️ هر جای دیگری که با «آیا job فعالی هست؟» تصمیم می‌گیرد همین آسیب‌پذیری را
> دارد — مثل `app/api/integration/pricing/resync/route.ts`.

## ۲. قیمت خرید دستی به هیچ پلتفرمی نمی‌رسید
`PATCH /api/integration/pricing` فقط `purchasePrice` را در دیتابیس می‌نوشت و
برمی‌گشت. قیمت جدید تا وقتی کسی دکمه‌ی «همگام‌سازی قیمت» را در فرم اتصال حسابان
نمی‌زد به هیچ پلتفرمی نمی‌رسید — و `SYNC_ALL_PRICE` **هیچ زمان‌بندی خودکاری ندارد**
(برخلاف `SYNC_ALL_STOCK`).

**رفع:** `pushPriceForMapping(mappingId)` در `pricing.ts` که PATCH بلافاصله
صدایش می‌زند. نتیجه (`pushed/failed/skipped`) در پاسخ برمی‌گردد و پنل نشانش می‌دهد.

## ۳. لاگ SYNC_ALL_PRICE دروغ می‌گفت
خلاصه‌ی کار همیشه `SUCCESS` با `pushedMappings: <تعداد نگاشت>` ثبت می‌شد — حتی
وقتی **هر ۱۴ ارسال** با «همگام‌سازی قیمت برای این اتصال غیرفعال است» رد شده بود.
تعداد نگاشت‌های پردازش‌شده با تعداد ارسال‌های موفق اشتباه گرفته شده بود.

**رفع:** `PricePushResult` واقعی از `pushMappingPrice` برمی‌گردد و وضعیت لاگ
`SUCCESS`/`PARTIAL`/`ERROR` از روی همان تعیین می‌شود.

## ۴. توکن وب‌هوک تپسی
مقایسه بدون `trim()` انجام می‌شد؛ یک فاصله یا `\n` چسبیده هنگام paste در فرم
ادمین همه‌ی فراخوانی‌ها را رد می‌کرد. `req.headers.get()` هم دو بار با املای
متفاوت صدا زده شده بود که بی‌اثر است (خودش case-insensitive است).

**رفع:** `normalizeToken()` (trim + حذف پیشوند `Bearer`) روی هر دو طرف، و ثبت
اثرانگشت ماسک‌شده‌ی توکن ارسالی (`طول=۳۲ abc…xyz`) در `requestData` لاگ.

> تپسی تا وقتی `succeed:true` نگیرد همان سفارش را **هر ۳ دقیقه تا ابد** دوباره
> می‌فرستد. این عمدی است (سفارش واقعی نباید خاموش دور ریخته شود) ولی یعنی هر
> خطای توکن به یک حلقه‌ی retry دائمی تبدیل می‌شود.

## درس کلی
هر سه ایراد از یک الگو می‌آیند: **خطا در سطح آیتم لاگ می‌شود ولی خلاصه‌ی کار
موفق گزارش می‌دهد.** موقع افزودن هر سینک جدید، وضعیت لاگ خلاصه باید از نتیجه‌ی
واقعی ارسال‌ها بیاید، نه از تعداد آیتم‌های پردازش‌شده.

> `resyncStockFromAccounting` هنوز همین الگو را دارد — چون همه‌ی
> `syncStockEnabled`ها روشن‌اند فعلاً کسی را گمراه نمی‌کند، ولی باید مثل قیمت اصلاح شود.

## ۵. توکن باسلام هرگز تجدید نمی‌شد (۴۰۱ روی همه‌چیز)
علامت در پنل: `FETCH_ORDERS` با

```
HTTP 401: {"errors":[{"code":0,"fields":[],"message":"authentication error"}],
           "http_status":401,"message":"Unauthorized"}
```

توکن باسلام `expires_in` دارد و منقضی می‌شود. فرم ادمین فیلد **Refresh Token**
را می‌گرفت و ذخیره می‌کرد، ولی `basalam.adapter.ts` هیچ‌جا از آن استفاده نمی‌کرد —
همه‌ی متدها مستقیم `credentials.accessToken` را در هدر می‌گذاشتند. یعنی از لحظه‌ی
انقضا، `FETCH_ORDERS` و `FETCH_PRODUCTS` و در نتیجه **ارسال قیمت** هم می‌مردند
(ارسال قیمت به snapshot تخفیف وابسته است و snapshot از `FETCH_PRODUCTS` می‌آید).

**رفع:** `authedFetch()` همه‌ی فراخوانی‌ها را می‌پوشاند؛ روی ۴۰۱ یک‌بار
`POST auth.basalam.com/oauth/token` با `grant_type=refresh_token` می‌زند، توکن
تازه را هم در حافظه و هم (رمزنگاری‌شده) روی اتصال ذخیره می‌کند و درخواست را
دوباره می‌فرستد. refreshهای همزمان از یک promise مشترک استفاده می‌کنند تا دو job
توکن همدیگر را باطل نکنند.

> refresh به `clientId` و `clientSecret` هم نیاز دارد — این دو به فرم اتصال
> باسلام اضافه شدند. تا وقتی پر نشوند، پیام خطای ۴۰۱ صریحاً می‌گوید کدام فیلد
> کم است، به‌جای اینکه فقط بدنه‌ی خام باسلام را نشان دهد.

## ۶. سینک موجودی، قیمت تازه را برمی‌گرداند (اسنپ‌شاپ)
`IntegPlatformProduct` فقط در «دریافت محصولات» نوشته می‌شد. پس بعد از یک
`SYNC_PRICE` موفق، snapshot هنوز **قیمت قدیمی** را داشت. اسنپ‌شاپ در هر PATCH
موجودی، `price` را هم اجباری می‌خواهد و آداپتور آن را از همین snapshot برمی‌داشت —
پس اولین سینک موجودیِ بعدی، قیمت تازه‌ارسال‌شده را به مقدار قبلی برمی‌گرداند.
علامتش برای کاربر: «قیمت‌ها بعضی وقت‌ها بروز نمی‌شود».

**رفع:** `core/snapshot.ts` با `recordPushedPrice()` / `recordPushedStock()` که
بعد از هر ارسال موفق در `pricing.ts`، `inventory.ts` و مسیرهای مستقیم
`SYNC_PRICE`/`SYNC_STOCK` در `worker.ts` صدا زده می‌شوند.

> ضمناً `SYNC_STOCK` و `SYNC_PRICE` در `worker.ts` نتیجه‌ی آداپتور را نادیده
> می‌گرفتند و job را `DONE` می‌کردند حتی وقتی پلتفرم قلم را رد کرده بود. حالا
> `failed` باعث throw و retry با backoff می‌شود.

## ۷. مکان‌نمای فید سفارش اسنپ‌شاپ زودتر از موعد جلو می‌رفت
`fetchOrders` مکان‌نما (`ordersCursor`) را **داخل خودش** و پیش از ذخیره‌ی
سفارش‌ها روی اتصال می‌نوشت. فید اسنپ‌شاپ یک‌طرفه است: اگر پردازش وسط کار
می‌شکست، job دوباره تلاش می‌کرد ولی مکان‌نما از آن رویدادها گذشته بود و آن
سفارش‌ها **برای همیشه** گم می‌شدند.

**رفع:** `commitOrdersCursor()` به `BaseAdapter` اضافه شد و `orders.ts` فقط
**بعد از** ثبت موفق ردیف‌های همان صفحه صدایش می‌زند.

> در همان حلقه یک محافظ هم اضافه شد: اگر پلتفرم `hasMore=true` بدهد ولی مکان‌نما
> تکراری یا خالی باشد، حلقه تا ابد می‌چرخید و job در `PROCESSING` گیر می‌کرد تا
> `reclaimStaleJobs` بعد از ۳۰ دقیقه آزادش کند. حالا حلقه می‌شکند و سقف
> `MAX_PAGES = 200` هم گذاشته شده.

## ۸. نگاشت خودکار عملاً مرده بود
سند (بخش ۸) می‌گوید بارکد ⇒ `1.0` و SKU ⇒ `0.95` و آستانه‌ی auto-approve
`>= 0.95` است. ولی `findBestMatch` فقط **عنوان** را مقایسه می‌کرد و سقف امتیازش
`0.85` (عنوان دقیق) بود — مسیر fuzzy حتی با امتیاز برند حداکثر `0.75`. یعنی شرط
`confidence >= 0.95` هرگز برقرار نمی‌شد و آن شاخه کد مرده بود: هر محصول، حتی با
SKU یکسان، به تأیید دستی می‌افتاد.

**رفع:** قوانین بارکد و SKU طبق سند پیاده شدند (`buildCodeIndex` روی
`Product.sku`). کدی که بین چند محصول تکراری است از ایندکس حذف می‌شود — نگاشت
خودکار روی کد مبهم یعنی وصل شدن به کالای اشتباه، که از نگاشت‌نشدن بدتر است.

## ۹. اسنپ‌شاپ: `special_price_stock` بزرگ‌تر از موجودی (۴۲۲)
در لاگ پروداکشن هفته‌ی گذشته ۱۲۵ بار:

```
HTTP 422 {"code":101002,"errors":[{"field_name":"products.0.special_price_stock", ...}]}
```

اسنپ‌شاپ `special_price_stock` را از snapshot قبلی می‌گرفت. در داده‌ی واقعی
همیشه `discountStock == stock` است (اسنپ‌شاپ خودش این‌طور نگه می‌دارد). وقتی
موجودی جدید کمتر از موجودی قبلی بود، بدنه‌ی PATCH یک `stock` کوچک و یک
`special_price_stock` بزرگِ کهنه داشت و اسنپ‌شاپ کل درخواست را رد می‌کرد.

**رفع:** `Math.max(0, Math.min(discount.stock, stock))` در `buildPayload`.

> فقط یک محصول (`r3V7Re`، موجودی ۵۹، تخفیف ۲۷٪) مسئول هر ۱۲۵ خطا بود — چون
> هر چرخه دوباره تلاش می‌شد.

## ۱۰. باسلام: یک قلم مردود، کل دسته‌ی ۵۰تایی را می‌کشت
`bulkUpdate` روی پاسخ ناموفق، خطا را برای **همه‌ی** `chunkIds` ثبت می‌کرد. ولی
۴۲۲ باسلام معمولاً تقصیر یک قلم است و شناسه‌اش را هم در متن می‌آورد:

```
محصولاتی که برای آنها تخفیف زمانمند تعیین شده است، با این ورژن از ای پی آی
قابل ویرایش نیستند. شناسه: 13088017
```

یعنی ۴۹ محصول سالم هم بی‌دلیل شکست‌خورده گزارش می‌شدند و دوباره تلاش می‌شد.

**رفع:** `blamedIds()` شناسه‌ها را از متن بیرون می‌کشد، همان‌ها failed ثبت
می‌شوند و بقیه‌ی دسته یک‌بار دیگر بدون آن‌ها ارسال می‌شود. ضمناً ۴۲۲ باسلام
پیام‌ها را زیر `messages` می‌فرستد نه `errors` — پارسر هر دو را می‌خواند.

> «تخفیف زمانمند قابل ویرایش نیست» یک محدودیت واقعی API باسلام است و با کد
> دور نمی‌خورد؛ تا وقتی کمپین تخفیف روی خود باسلام فعال باشد آن محصول با این
> نسخه‌ی API ویرایش نمی‌شود. مرتبط با بحث «منبع حقیقت قیمت» در کارهای باز.
