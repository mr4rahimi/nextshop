...
| ۷ | گزارش‌ها و داشبورد | ✅ انجام شد (۲.۵۹.۰) |
| ۸ | انتقال داده‌ی مای مونتا از حسابان (اکسل یا API) — **آخر کار** | ⏳ |
| ۹ | بستن سال مالی (**بعدی** — تصمیم ۱۵: پیش از انتقال)، سپس سامانه‌ی مودیان | ⏳ |
...
| ۲.۵۹.۰ — فاز ۷ | ✅ ۱۴۰۵/۰۷/۰۴ (بی‌مهاجرت؛ مسیرهای تازه ۳۰۷/۴۰۱، لاگ خطا خالی) | فرمان‌ها به کاربر داده شد |
...
6. **فاز بعدی: بستن سال مالی** (بخش ۱۷، نیمه‌ی اول فاز ۹ — تصمیم ۱۵ آن را پیش از انتقال مای مونتا می‌گذارد). سند اختتامیه با `source = CLOSING` ساخته شود — گزارش‌های فاز ۷ (`profitLoss`، `expenseReport`، داشبورد) همین منبع را کنار می‌گذارند؛ افتتاحیه‌ی سال بعد `OPENING` است و `readOpening` آن را می‌خواند (کیف پول جدا، تصمیم ۲۲). ترازنامه و تراز آزمایشی بعد از بستن باید همان عدد قبل را بدهند — آزمونش را از `scratchpad/reports-test.ts` بسازید.
...
| ۲۵ | گزارش‌ها | یک route (`/api/admin/accounting/reports/[kind]`) و یک ماژول در `lib/accounting/reports/` برای هر خانواده. مبلغ‌ها از `AccVoucherLine`؛ **استثنا:** سود کالا (فروش از ردیف فاکتور، بها از `AccStockMove.totalCost`) و ارزش موجودی (`AccStock` × سهم `AccProductCost.totalValue`). بخش‌های سود و زیان از جای حساب‌های سیستمی در درخت (`SALES` ← کلِ فروش، `COGS` ← گروه بهای تمام‌شده) پیدا می‌شوند، نه از کد. سند `CLOSING` از سود و زیان و هزینه‌ها بیرون است | حساب تازه‌ی کسب‌وکار خودش در بخش درست می‌نشیند؛ بستن سال سود را صفر نشان نمی‌دهد |
| ۲۶ | دسترسی گزارش | `ACC_REPORTS`؛ سود و زیان، ترازنامه، تراز آزمایشی، سود کالا و ارزش موجودی `ACC_COST_VIEW` هم می‌خواهند (۴۰۳). داشبورد سود ناخالص/خالص را بی `ACC_COST_VIEW` از سرور نمی‌فرستد | بهای خرید حساس‌ترین عدد است (بخش ۱۴) |
...
| `ACC_REPORTS` | گزارش‌های مالی ✅ (فاز ۷؛ تصمیم ۲۶) |
...
lib/accounting/
  ...
  reports/                ← common (ایندکس درخت، جمع هر حساب)، trial، statements (سود و زیان، ترازنامه)، aging، profit، ops (ارزش افزوده، هزینه، گردش خزانه، ارزش موجودی)، dashboard
...
31. **COGS دفتر ≠ بهای کاردکس فروش‌ها.** گزارش «سود کالا» فقط بهای خروج فروش منهای
    برگشت را دارد؛ حساب COGS اختلاف قیمت برگشت از خرید (فاز ۴) و … را هم دارد. روی
    داده‌ی محلی: ۲۸٬۸۰۸ − ۱۲٬۸۲۴ = ۱۵٬۹۸۴ (کاردکس) و ۹٬۸۳۵ برگشت از خرید = ۲۵٬۸۱۹ (دفتر).
    این اختلاف باگ نیست؛ در متن گزارش و راهنما گفته شده.
32. **پالت نمودار سبز/قرمز ندارد.** فروش آبی، سود ناخالص فیروزه‌ای، هزینه نارنجی
    (`SERIES` در `reports/kit.tsx`، با `validate_palette.js --pairs all` در هر دو حالت).
    سبز و قرمز برای کوررنگی قرمز-سبز یکی‌اند (ΔE ۲٫۷). فیروزه‌ای روشن کنتراست زیر ۳:۱
    دارد — برای همین نمودار داشبورد «نمای جدول» دارد؛ حذفش نکنید.
33. **چاپ گزارش** با CSS داخل `ReportFrame` است (همه‌چیز جز `.acc-report` پنهان)، نه
    مسیر `/print` جدا. هر گزارش تازه باید داخل `ReportFrame` باشد.
34. **breadcrumb با کلید «پدر/بخش».** `BREADCRUMB_LABELS` حالا اول `"accounting/reports"`
    را نگاه می‌کند، بعد `reports` (که «گزارش عملکرد» پنل است).
...
### فاز ۷ — گزارش‌ها و داشبورد ✅ انجام شد (۲.۵۹.۰)

**آنچه ساخته شد:**

| چیز | کجا |
|---|---|
| موتور | `lib/accounting/reports/`: `trial.ts` (شش عدد هر ردیف؛ سطح گروه/کل/معین/تفصیلی با زنجیره‌ی پدران)، `statements.ts` (`profitLoss` با مقایسه، `balanceSheet` با `balanced`)، `aging.ts` (از `invoiceOpenAmounts`؛ عمر از سررسید یا تاریخ)، `profit.ts` (کالا/دسته/کانال)، `ops.ts` (`vatReport`، `expenseReport`، `treasuryFlow`، `inventoryValue` با تطبیق کاردکس و دفتر)، `dashboard.ts` (`monthDashboard`) |
| تاریخ | `dates.ts` + `jalaliMonthBounds`، `previousRange`، `daysBetween` |
| API | `GET /api/admin/accounting/reports/[kind]` — `pl`، `balance`، `trial`، `aging`، `profit`، `vat`، `expenses`، `treasury`، `inventory`؛ `summary` + `month` و `can` |
| رابط | `components/admin/accounting/reports/`: `kit.tsx` (`ReportFrame` با اکسل و چاپ، `useReport`، `Amt`، `Change`، `Table`، `useIsDark`، `SERIES`)، `ReportsHub` و یک کامپوننت برای هر گزارش؛ `InternalDashboard` + «این ماه» (کارت‌های افقی در موبایل)، نمودار روزانه با `MultiLineChart` پنل و نمای جدول، «کارهای مانده» |
| صفحه‌ها | `/admin/accounting/reports` و `/reports/{pl,balance,trial,aging,profit,vat,expenses,treasury,inventory}` |
| جاهای دیگر | زبانه و منوی «گزارش‌ها»؛ `InventoryClient` فیلتر را از `?filter=` می‌خواند؛ `SOURCE_LABELS` + `WALLET` (در فاز ۶ جا مانده بود) |
| مجوز | + `ACC_REPORTS` (تصمیم ۲۶) |
| راهنما | + `accountingReports`، `accountingPL`، `accountingBalanceSheet`، `accountingTrial`، `accountingAging`، `accountingProfit`، `accountingVatReport`، `accountingExpenseReport`، `accountingTreasuryReport`، `accountingInventoryValue`؛ `accounting` به‌روز |

**عمداً انجام نشد:**
- ارسال صورت‌حساب شخص با پیامک/لینک برای مشتری (بخش ۱۱) — صورت‌حساب همان صفحه‌ی شخص است و چاپ می‌شود.
- نمودار در همه‌ی گزارش‌ها — فقط داشبورد (روزانه) و هزینه‌ها (میله‌ای)؛ بقیه جدول‌اند.
- خروجی xlsx واقعی — CSV با BOM که اکسل مستقیم باز می‌کند؛ کتابخانه‌ی تازه اضافه نشد.

**آزمون:** `scratchpad/reports-test.ts` (۲۰ بررسی: تراز آزمایشی در چهار سطح و بازه‌ی میانی شش‌ستونی، ترازنامه تراز، سود خالص = سود ترازنامه، سود ناخالص، ردیف فروش = مانده‌ی حساب، مقایسه، هزینه = هزینه‌ی سود و زیان، سنی بدهی = جمع فاکتورهای باز، گردش خزانه = مانده، کاردکس = حساب موجودی و جمع انبارها و دسته‌ها، سود کالا و کانال یک جمع، ارزش افزوده = دفتر، سری روزانه‌ی داشبورد = عدد ماه) + `reports-api.mjs` (۲۵ بررسی) + کاربر محدود (بی `ACC_COST_VIEW`: پنج گزارش ۴۰۳، داشبورد بی سود) + تصویر صفحه‌ها در دسکتاپ و موبایل. معیار فاز روی داده‌ی محلی برقرار است؛ دمو بی‌داده است (فاز ۶).
