...
| ۳ | کالا و انبار: چند انبار، کاردکس، میانگین موزون | ✅ انجام شد (۲.۵۵.۰ — محلی) |
| ۴ | فاکتورها: فروش، خرید، برگشتی، پیش‌فاکتور، چاپ رسمی | ⏳ |
| ۵ | خزانه: صندوق، بانک، کارتخوان، دریافت و پرداخت، چک | ⏳ |
| ۶ | هزینه‌ها و وصل شدن اقساط، کیف پول و پورسانت | ⏳ |
...
6. **فاز بعدی: ۴ — فاکتورها** (فروش سایت/تلفنی/بازارگاه از کاردکس رد شود؛ تله‌ی ۱۵).
...
| `COMMISSION_PAID` | `StaffPayout` | هزینه‌ی پورسانت |
| `WALLET_USED` | پرداخت سفارش از کیف پول | کاهش بدهی کیف پول |
| `MARKETPLACE_SETTLED` | واریز بازارگاه (دستی) | دریافت + کارمزد |
...
| **شارژ کیف پول** | خزانه | پیش‌دریافت/کیف پول مشتریان (شخص) |
| **پرداخت از کیف پول** | کیف پول مشتریان (شخص) | AR (شخص) |
| **تسویه‌ی پورسانت** | هزینه‌ی پورسانت فروش (شخص=کارمند) | خزانه |
| **تسویه‌ی بازارگاه** | بانک ← واریزی<br>کارمزد بازارگاه ← کسر شده | AR (شخص=بازارگاه) |
...
lib/accounting/
  ...
  treasury/               ← دریافت، پرداخت، هزینه، انتقال، تخصیص
  cheques/                ← ماشین وضعیت و سندهای هر گذار، یادآوری
...
15. **`Product.stock` تفاضلی است، نه مطلق** — تا فاز ۴ فروش سایت هنوز مستقیم از
    `Product.stock` کم می‌کند و از کاردکس نمی‌گذرد؛ نوشتن مطلق جمع کاردکس آن کسرها
    را پاک می‌کرد. هر حرکت، تفاضلش در انبار قابل فروش را به `Product.stock` اضافه
    می‌کند (هرگز زیر صفر). **استثنا: موجودی اول دوره** که مطلق است. در فاز ۴ که
    فروش از کاردکس رد شد، کسر مستقیم `deductStockForOrderItems` در حالت داخلی
    باید خاموش شود، وگرنه دو بار کم می‌شود.
...
### فاز ۴ — فاکتورها
- فروش، خرید، برگشتی‌ها، پیش‌فاکتور و تبدیل؛ `InternalProvider` برای `SALE_*` و
  `PURCHASE_*`؛ ساخت خودکار از سفارش سایت/تلفنی/بازارگاه؛ سه قالب چاپ + PDF.
- ✅ **وقتی:** یک سفارش سایت از پرداخت تا مرجوعی، فاکتور، کاردکس و سندهای درست
  دارد و تراز آزمایشی صفر است؛ فاکتور رسمی با مهر چاپ می‌شود.

### فاز ۵ — خزانه و چک
- دریافت/پرداخت چندروشی با تخصیص، انتقال وجه، درگاه و کارتخوان، `Payment` خودکار؛
  چک با ماشین وضعیت کامل، دسته‌چک، یادآوری سررسید و کار پیگیری.
- ✅ **وقتی:** هر گذار چک سند درست دارد و گذار غیرمجاز رد می‌شود؛ پرداخت آنلاین
  خودکار به فاکتور تخصیص می‌یابد.

### فاز ۶ — هزینه و اتصال بخش‌های موجود
- ثبت هزینه (نقد/نسیه، چندسرفصلی)؛ اقساط اعتباری، کیف پول، `StaffPayout`، تسویه‌ی
  بازارگاه با کارمزد.
- ✅ **وقتی:** پرداخت قسط در کارتابل طلب شخص را در حسابداری کم می‌کند.
...
| # | سؤال | فاز |
|---|---|---|
| ۱ | کسر موجودی سفارش تلفنی/اعتباری دقیقاً در کدام گذار است؟ (برای زمان فاکتور) | ۴ |
