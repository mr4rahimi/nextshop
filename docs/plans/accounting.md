...
6. **فاز بعدی: ۶ — هزینه و اتصال بخش‌های موجود.** ثبت هزینه (نقد/نسیه، چندسرفصلی) روی همان `AccMoneyDoc` با نوع تازه‌ی `EXPENSE`؛ اقساط اعتباری (`payInstallment`)، کیف پول (`WALLET` که فاز ۵ عمداً رد می‌کند)، `StaffPayout` و تسویه‌ی بازارگاه. رویدادها با `emitAccEventSafe` و handler در `providers/internal.ts`.
...
lib/accounting/
  ...
  cash/                   ← docs.ts (دریافت، پرداخت، انتقال)، allocation.ts، cheques.ts (ماشین وضعیت)، channel.ts (Payment سایت)، reminders.ts
