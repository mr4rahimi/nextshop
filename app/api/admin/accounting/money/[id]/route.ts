...
/** هزینه با `ACC_EXPENSE` نوشته می‌شود، بقیه با `ACC_TREASURY` */
const writePerm = (kind: string) => (kind === "EXPENSE" ? "ACC_EXPENSE" : "ACC_TREASURY");

export async function GET(_req: Request, { params }: Params) {
  const guard = await requirePermission(["ACC_VIEW", "ACC_TREASURY", "ACC_EXPENSE"]);
  ...
  const doc = await prisma.accMoneyDoc.findUnique({
    ...
    include: {
      ...
      lines: { orderBy: { seq: "asc" }, include: { account: { select: { id: true, code: true, name: true } } } },
  ...
  const open = doc.partyId && (doc.kind === "RECEIPT" || doc.kind === "PAYMENT") && doc.status === "POSTED" ? await openInvoicesOf(prisma, doc.partyId, doc.kind === "RECEIPT" ? "sales" : "purchase") : [];
  ...
  return NextResponse.json(
    serialize({
      ...
      allocatable,
      can: { write: can(guard.access, writePerm(doc.kind)), pay: can(guard.access, "ACC_TREASURY") },
    }),
  );
}
...
export async function POST(req: Request, { params }: Params) {
  const guard = await requirePermission(["ACC_TREASURY", "ACC_EXPENSE"]);
  if (!guard.ok) return NextResponse.json({ error: guard.error }, { status: guard.status });
  try {
    const { id } = await params;
    const kind = (await prisma.accMoneyDoc.findUnique({ where: { id }, select: { kind: true } }))?.kind;
    if (!kind) throw new AccError("پیدا نشد", 404);
    if (!can(guard.access, writePerm(kind))) throw new AccError("دسترسی ندارید", 403);
