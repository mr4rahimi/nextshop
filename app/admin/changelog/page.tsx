...
const VERSIONS = [
...

  {
    ...
    description: "نمایش صفحات فرود سئو به‌صورت لینک‌های اسلایدی در صفحه دسته‌بندی، ابزار استقرار خودکار با بکاپ‌گیری و بازگردانی، و رفع چند باگ عملکردی و دسترسی‌پذیری",
    changes: [
      ...
      { type: "fix", text: "رفع باگ رفرش مکرر صفحه دسته‌بندی که باعث درخواست‌های پیاپی به سرور می‌شد" },
      { type: "fix", text: "رفع خطاهای پیاپی اتصال Redis در لاگ سرور و کندی فرآیند ساخت پروژه" },
      { type: "fix", text: "اصلاح نشانی‌های نامعتبر ویژگی‌ها که مانع فیلتر صحیح محصولات می‌شد" },
    ],
  },
    ...
    date: "1405/05/13",
    tag: "minor",
    title: "سئوی فیلترها، صفحات فرود و رفع باگ جستجوی موبایل",
    description: "همگام‌سازی فیلترهای دسته‌بندی با آدرس صفحه، امکان ساخت صفحات فرود سئو با آدرس ثابت و قابل ایندکس، رندر سمت سرور نتایج فیلترشده، و رفع چند باگ مهم در جستجو و صفحه‌بندی محصولات",
    changes: [
      ...
      { type: "fix", text: "رفع باگ جستجو در موبایل — آدرس نادرست API باعث می‌شد همیشه «نتیجه‌ای یافت نشد» نمایش داده شود" },
      { type: "fix", text: "افزودن جستجوی زنده (حین تایپ) به صفحه جستجو در موبایل" },
      { type: "fix", text: "رفع باگ نمایش تنها یک محصول در API محصولات هنگام عدم ارسال تعداد در هر صفحه" },
      { type: "fix", text: "اصلاح فیلتر چند ویژگی — انتخاب «لیزری» و «تک کاره» حالا اشتراک می‌گیرد نه اجتماع" },
      { type: "fix", text: "رفع تولید نشانی نامعتبر برای ویژگی‌های فارسی در پنل ادمین" },
    ],
  },
   {
    version: "2.10.0",
    ...
    changes: [
      ...
      { type: "improvement", text: "کسر موجودی سفارش سایت فقط پس از پرداخت موفق — جلوگیری از کسر پیش از پرداخت و کسر دوباره" },
      ...
      { type: "fix", text: "رفع خطای ۴۰۱ اتصال ترب — تطبیق audience توکن با دامنه واقعی پشت پراکسی" },
      { type: "fix", text: "رفع خطای دریافت سفارش باسلام — حذف پارامتر نامعتبر مرتب‌سازی (sort)" },
      { type: "fix", text: "رفع خطای BigInt در APIهای نگاشت و محصولات پلتفرم" },
      { type: "fix", text: "رفع کسر دوباره موجودی هنگام تأیید سفارش در پنل ادمین" },
    ],
  },
