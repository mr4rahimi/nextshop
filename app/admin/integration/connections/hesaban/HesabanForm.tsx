...
export default function HesabanForm({ existingConnection }: Props) {
  ...
  const [testResult, setTestResult] = useState<{ success: boolean; message?: string; shopInfo?: Record<string, unknown> } | null>(null);
  ...
  if (storageId.trim()) credentials.storageId = storageId.trim();

  async function handleTest() {
    if (!token.trim()) {
      setTestResult({ success: false, message: "توکن را وارد کنید" });
      return;
    }
    setTesting(true);
    setTestResult(null);
    try {
      const res = await fetch("/api/integration/connections/test", {
        ...
        body:    JSON.stringify({ platformCode: "hesaban", credentials }),
      });
      const data = await res.json() as { success: boolean; message?: string; shopInfo?: Record<string, unknown> };
      setTestResult(data);
    } catch {
  ...
  async function handleSync(type: "SYNC_ALL_STOCK" | "SYNC_ALL_PRICE") {
    setSyncing(type);
    setSyncMsg(null);
    try {
      const res = await fetch("/api/integration/sync", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ platformCode: "hesaban", type }),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#0f1117] rounded-2xl border border-gray-200 dark:border-white/[0.06] p-5 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Bearer token از پنل وب‌حسابان"
          ...
          <p className="text-[11px] text-gray-400 mt-1.5">
            از پنل وب‌حسابان → تنظیمات → API → توکن دسترسی دریافت کنید
          </p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="شناسه انبار — خالی = همه انبارها"
            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.03] text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-colors font-mono"
            dir="ltr"
          />
        </div>
        ...
        {testResult && (
          <div className={`p-3 rounded-xl text-sm ${
            ...
            {testResult.success && testResult.shopInfo && (
              <pre className="mt-2 text-[10px] opacity-60 overflow-auto max-h-24 font-mono">
                {JSON.stringify(testResult.shopInfo, null, 2)}
              </pre>
            )}
          </div>
        )}
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">موجودی کالاها از وب‌حسابان به فروشگاه منتقل می‌شود</p>
            </div>
          </label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">قیمت فروش از وب‌حسابان به فروشگاه منتقل می‌شود</p>
            </div>
          </label>
        </div>
      ...
      {existingConnection?.status === "CONNECTED" && (
        <div className="bg-white dark:bg-[#0f1117] rounded-2xl border border-gray-200 dark:border-white/[0.06] p-5 space-y-4">
          ...
          <p className="text-xs text-gray-400">برای sync فوری بدون انتظار برای زمان‌بندی خودکار</p>
          ...
          <div className="flex flex-wrap gap-3">
            {syncStock && (
              ...
              >
                {syncing === "SYNC_ALL_STOCK" ? "در صف..." : "همگام‌سازی موجودی"}
              </button>
            )}
            {syncPrice && (
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-bold hover:opacity-80 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {syncing === "SYNC_ALL_PRICE" ? "در صف..." : "همگام‌سازی قیمت"}
              </button>
            )}
          </div>
        </div>
