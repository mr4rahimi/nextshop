...
> آخرین به‌روزرسانی: 1405/04/27 — فاز ۰ تکمیل شد
> وضعیت: فاز ۰ تمام · فاز ۱ آماده شروع
...
## ۴. مرجع API ایران پیامک

مستندات کامل: `https://docs.iranpayamak.com/` — کپی محلی در پوشه `iranpayamak/`
Base URL: `https://api.iranpayamak.com` · احراز هویت: هدر `Api-Key`

| کاربرد | متد و مسیر | نکته |
| --- | --- | --- |
| پترن (OTP، وضعیت سفارش) | `POST /ws/v1/sms/pattern` | در حال استفاده در `lib/sms.ts` |
| ارسال انبوه یک متن | `POST /ws/v1/sms/simple` | آرایه `recipients` |
| **ارسال شخصی‌سازی‌شده** | `POST /ws/v1/sms/keywords` | متغیر `%var%`، هر گیرنده مقدار خودش |
| متن متفاوت برای گروه‌ها | `POST /ws/v1/sms/peer-to-peer` | آرایه `peers` |
| ارسال از فایل اکسل | `POST /ws/v1/sms/simple-file` | برای ورود دستی |
| ارسال تست به مالک | `POST /ws/v1/sms/sample` | برای دکمه «ارسال تست» کمپین |
| محاسبه هزینه | `POST /ws/v1/sms/calculate-cost/peer-to-peer` | قبل از اجرای کمپین |
| موجودی حساب | `GET /ws/v1/account/balance` | `balanceAmount` و `balanceCount` |
| ارسال منطقه‌ای | `GET /ws/v1/sms/postal/get-receiver-count` | بر اساس کد پستی، جنسیت، سن |
| دفترچه تلفن | `GET /ws/v1/number_bank` | فعلاً استفاده نمی‌کنیم |

**ساختار پاسخ مشترک:**
```json
{ "status": "success", "data": 12345, "messages": null }
```
`data` شناسه‌ی درخواست ارسال است و باید در `SmsMessage.providerMsgId` ذخیره شود.

**پارامترهای مشترک:** `line_number` · `number_format` (`english` | `persian`) · `schedule` (اختیاری، زمان آینده)

### تصمیم معماری: واحد کار = دسته، نه پیام

چون `/ws/v1/sms/keywords` در یک درخواست چند گیرنده با متغیرهای متفاوت را می‌پذیرد، هر job در صف یک **دسته** (پیش‌فرض ۲۰۰ گیرنده) است، نه یک پیامک تکی. برای کمپین ۵۰۰۰ نفره فقط ۲۵ درخواست به پنل زده می‌شود.

### هنوز مستند نشده

- endpoint **وضعیت تحویل** پیام‌ها → بلاک‌کننده بخش گزارش تحویل در فاز ۲
- endpoint **پیام‌های دریافتی / منشی پیامک** → بلاک‌کننده پردازش خودکار «لغو» در فاز ۳
...
### فاز ۰ — زیرساخت ✅
- [x] افزودن `redis` به `docker-compose.yml` (پورت `63791`، `appendonly`، `noeviction`)
...
- [x] متغیرهای محیطی: `REDIS_URL`، `CLUB_WORKER_CONCURRENCY`، `SMS_BATCH_SIZE`
...
| ۶ | ارسال متن آزاد و انبوه | ✅ `/sms/simple` و `/sms/keywords` |
| ۷ | استعلام اعتبار | ✅ `GET /ws/v1/account/balance` |
| ۸ | **وضعیت تحویل** — endpoint؟ | 🔴 بلاک‌کننده گزارش تحویل در فاز ۲ |
| ۹ | **منشی پیامک / پیام‌های دریافتی** — endpoint یا webhook؟ | 🔴 بلاک‌کننده «لغو» خودکار در فاز ۳ |
| ۱۰ | امتیاز و کیف پول یکی شوند یا جدا بمانند؟ (پیشنهاد: جدا) | ⏳ فاز ۴ |
| ۱۱ | مدل دستگاه شماره‌گیر مشخص است؟ | ⏳ فاز ۶ |

**نکته:** موارد ۸ و ۹ فاز ۱ را بلاک نمی‌کنند.
...
| --- | --- | --- | --- |
| ۰ — زیرساخت | ✅ تکمیل‌شده | 1405/04/27 | Redis + BullMQ + نرمال‌ساز شماره + worker |
| ۱ — عضویت | 🟡 آماده شروع | — | — |
| ۲ — پیامک | ⬜ شروع نشده | — | — |
