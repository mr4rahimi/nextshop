...
### ✅ فاز ۵ — صفحه نگاشت محصولات (بازطراحی کامل)
- مدل `IntegPlatformProduct` اضافه شد — کَش خام محصولات هر پلتفرم
- Worker آپدیت شد: `fetchAndMatch` حالا محصولات را در `IntegPlatformProduct` upsert می‌کند
- API جدید: `GET /api/integration/platform-products` — محصولات پلتفرم با وضعیت نگاشت
- صفحه نگاشت بازطراحی شد:
  - تب جداگانه برای هر پلتفرم: همه محصولات پلتفرم نمایش داده می‌شود
  - نشانگر وضعیت: 🟢 نگاشت شده / 🟡 پیشنهاد در انتظار / 🔴 بدون نگاشت
  - تأیید یا رد پیشنهاد سیستم بدون خروج از صفحه
  - نگاشت دستی: modal جستجوی محصول فروشگاه
  - تب «نگاشت‌های تأیید‌شده» — جدول کامل `IntegProductMapping`
- معماری ظرف اصلی حفظ شد: `IntegProductMapping` منبع sync موجودی/قیمت است
