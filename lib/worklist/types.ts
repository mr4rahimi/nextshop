...
/**
 * فهرست پشتیبانِ باربری — **فقط وقتی به کار می‌رود که هیچ روش ارسالی تعریف
 * نشده باشد**. منبع اصلی جدول `ShippingMethod` است (`useInWorklist`).
 *
 * ⚠️ چیزی به این آرایه اضافه نکنید. باربری تازه در `/admin/shipping` تعریف
 * می‌شود؛ آرایه‌ی کد برای هر کسب‌وکارِ تازه فقط نقطه‌ی شروع است.
 */
export const FALLBACK_CARRIERS = ["تیپاکس", "چاپار", "بار هوایی", "ترمینال", "پست", "پیک موتوری"];
